    ...
    <div class="relative bg-white border shadow-lg p-5 rounded-xl overflow-hidden">
        <div class="flex flex-col laptop:flex-row gap-4 laptop:items-center laptop:justify-between py-5">
            <div class="flex flex-row justify-between items-center laptop:items-start laptop:flex-col">
                ...
                <span class="text-gray-400 font-semibold text-sm">{{ $alumnis->total() }} Alumni</span>
            </div>
            <div class="flex items-center laptop:w-auto w-full">
                <label for="searchAlumniList" class="sr-only">Search</label>
                <div class="flex items-center gap-5">
                    <div class="relative w-full">
                        <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                            <svg aria-hidden="true" class="w-5 h-5 text-gray-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"></path></svg>
                        </div>
                        <input type="text" id="searchAlumniList" wire:model.debounce.500ms="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-full focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 " placeholder="Search" required>
                    </div>
                </div>
            </div>
        </div>
                ...
                <tbody>
                    @forelse ($alumnis as $alumni)
                        <tr class="bg-white border-b">
                            <td class="py-4 px-6">
                                <h1 class="font-semibold truncate">{{ $loop->iteration + $alumnis->firstItem() - 1}}</h1>
                            </td>
                            <td class="py-4 px-6">
                            ...
                            <td class="py-4 px-6">
                                <h1 class="font-semibold truncate">{{ $alumni->domicile }}</h1>
                            </td>
                            <td class="py-4 px-6">
                                <h1 class="font-semibold truncate">{{ $alumni->phone }}</h1>
                            </td>
                            <td class="py-4 px-6">
                                <h1 class="font-semibold truncate">{{ $alumni->generation }}</h1>
                            </td>
                            <td class="py-4 px-6">
                                <div class="flex items-center gap-4">
