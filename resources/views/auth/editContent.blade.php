...
<body class="bg-white">
    ...
    <section class="px-5 laptop:px-20 pt-5" data-aos="fade-right" data-aos-anchor-placement="top-bottom" data-aos-duration="2000">
        <h1 class="text-4xl font-bold uppercase">Edit Content</h1>
        <div class="border-b-2 border-b-black w-full"></div>
    </section>

    <section class="px-5 laptop:px-20 mt-5">
        <form action="{{ route('editContent.update', ['id' => $content->id]) }}" enctype="multipart/form-data" method="post" >
            @csrf
            @method('PATCH')
            <div class="mb-6 space-x-10 flex items-center w-full">
                <div class="space-x-3 w-full flex items-center">
                    <label for="title" class="font-semibold">Title</label>
                    <input type="text" name="title" value="{{ $content->title }}" class="rounded-xl bg-gray-200 border-0 w-full" id="title" placeholder="Title" required>
                </div>
                <div class="space-x-3 w-full flex items-center">
